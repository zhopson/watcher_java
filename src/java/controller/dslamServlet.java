...
public class dslamServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean sel_no_binds = false, sel_arch = false;
        String st_d = null, en_d = null;
...
//                        }
                    ...
                    if (resultList.size() != 0) {
                        for (Iterator iterator = resultList.iterator(); iterator.hasNext();) {
                            ...
                            sb.append("\"" + next.getPortStatus() + "\",");
                            sb.append("\"" + next.getIpLine() + "\",");
                            sb.append("\"" + next.getProfileLine() + "\",");
                            sb.append("\"" + next.getProfileIptv() + "\",");
                            sb.append("\"" + next.getVlanLine() + "\",");
                            sb.append("\"" + next.getPvcLine() + "\",");
                            sb.append("\"" + next.getVlanIptv() + "\",");
                            sb.append("\"" + next.getPvcIptv() + "\",");
                            sb.append("\"" + next.getEdsnr() + "\",");
                            sb.append("\"" + next.getEda() + "\",");
                            sb.append("\"" + next.getEusnr() + "\",");
                            sb.append("\"" + next.getEua() + "\",");
...
//                            response.setStatus(HttpServletResponse.SC_NO_CONTENT);
            ...
            } else if ("get_dslam_stat".equals(action)) {
                ...
                Object[] res_stat = (Object[]) query.setParameter(1, host).setParameter(2, begin_date).setParameter(3, end_date).getSingleResult();
//                        List resultListS = query.setParameter(1, host).setParameter(2, begin_date).setParameter(3, end_date).getResultList();
//                        if (resultListS.size() != 0) {
//                            for (Iterator iterator = resultListS.iterator(); iterator.hasNext();) {
//                                Object[] next = (Object[]) iterator.next();
...
//                            }
//                        }
//                        response.setAttribute("total_stat", res_stat[0]);
//                        response.setAttribute("acting_stat", res_stat[1]);
//                        response.setAttribute("acted_stat", res_stat[2]);
//                        response.setAttribute("deacted_stat", res_stat[3]);
//                        response.setAttribute("nobinds_stat", res_stat[4]);
...
//                            response.getWriter().write("<dslam_stat>"
//                                    +"<status>"
//                                    + 1
//                                    + "</status>"
//                                    +"<total_stat>"
...
//                                    +"</dslam_stat>");                        
                ...
                if (sel_arch) { // поиск в архиве
                    ...
                    begin_date = fmt.parse(date_s);
                    end_date = fmt.parse(date_e);
                } else { // поиск в текущих значениях опроса портов dslam за текущую дату
                    Date dd = new Date();
                    String dt_s = fmt.format(dd);
                    dt_s = dt_s.substring(0, 10);
                    String date_s = dt_s + " 00:00:00";
